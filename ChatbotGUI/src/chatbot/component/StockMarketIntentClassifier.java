...
public class StockMarketIntentClassifier {
	
	private static String[] intentDictionary;
	
	public StockMarketIntentClassifier() {
		initializeIntentDictionary();
	}
	
	/**
	 * Create a dictionary of intents (under Stock Market domain)  
	 */
	...
	private Double[] calculateIntentScores(String nowInputText) {
		...
		for(String stockmarketKeyword: price_intent) {
		...
		for(String stockmarketKeyword: good_stock_intent) {
			for(String token: tokenList) {
				if(token.equals(stockmarketKeyword)) {
		...
		for(String stockmarketKeyword: bad_stock_intent) {
			for(String token: tokenList) {
				if(token.equals(stockmarketKeyword)) {
